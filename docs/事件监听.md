...
**本文档概述 [botpy 的事件监听](../botpy/flags.py)**
...
群聊、C2C 消息、机器人管理和入群申请事件使用 `public_messages` Intent：
...
| 对应函数 | 说明 |
| --- | --- |
| `on_group_at_message_create(self, message: GroupMessage)` | 收到群内 @ 机器人消息 |
| `on_c2c_message_create(self, message: C2CMessage)` | 收到 C2C 消息 |
| `on_group_add_robot(self, event: GroupManageEvent)` | 机器人加入群聊 |
| `on_group_del_robot(self, event: GroupManageEvent)` | 机器人退出群聊 |
| `on_group_msg_receive(self, event: GroupManageEvent)` | 群聊消息接收开启 |
| `on_group_msg_reject(self, event: GroupManageEvent)` | 群聊消息接收关闭 |
| `on_group_join_request(self, event: GroupJoinRequestEvent)` | 用户申请加入群聊 |

群成员事件使用 `group_member_event` Intent：
...
| 对应函数 | 说明 |
| --- | --- |
| `on_group_member_add(self, event: GroupMemberEvent)` | 群成员加入 |
| `on_group_member_remove(self, event: GroupMemberEvent)` | 群成员退出或被移出 |

群管理事件对象和审批示例见 [群管理 API 与事件](./GROUP_MANAGEMENT.md)。
...
| group_member_event      | 群成员事件（`1 << 24`）                       |
| public_messages         | 群聊、C2C 消息、机器人管理和入群申请事件（`1 << 25`） |
